...
namespace GeneralAI
{
    public class DataCollect
    {
        ...
        {
            if (Program.panel.connection.isConnected)
            {
                if (!isCollecting)
                {
                    Program.panel.connection.consumerVisual1.Received += new BasicDeliverEventHandler(consumerVisual1_Received);
                    Program.panel.connection.consumerVisual2.Received += new BasicDeliverEventHandler(consumerVisual2_Received);
                    Program.panel.connection.consumerVisual3.Received += new BasicDeliverEventHandler(consumerVisual3_Received);
                    Program.panel.connection.consumerAudio1.Received += new BasicDeliverEventHandler(consumerAudio1_Received);
                    Program.panel.connection.consumerAudio2.Received += new BasicDeliverEventHandler(consumerAudio2_Received);
                    Program.panel.connection.consumerAudio3.Received += new BasicDeliverEventHandler(consumerAudio3_Received);
                    Program.panel.connection.consumerAvatar1.Received += new BasicDeliverEventHandler(consumerAvatar1_Received);
                    isCollecting = true;
                    return true;
                }
                else
