// image_filter.cpp : Defines the entry point for the console application.
//
...
/* November 11, 2002                                                          */
/******************************************************************************/
/* Purpose : Image Filtering Program                                          */
/******************************************************************************/
...
void image_filter(int pict[][IM_SIZE], int r, int c, double filter[][FILTER_SIZE],int new_pict[][IM_SIZE], int npes, int myrank); /* 8/4/2013 */
...
/******************************************************************************/
/* Source Code:                                                               */
void image_filter(int pict[][IM_SIZE], int r, int c, double filter[][FILTER_SIZE],int new_pict[][IM_SIZE],int npes,int myrank)
{
	double coeff = 0;
	double sum;
	int i, j, m, n;

	if(myrank==0) /* 8/4/2013 */
	{
		/*  compute coefficient                                                       */
        ...
        for (i = 1; i < npes; i++) {//for each slave other than the master
            portion = (r / (npes - 1)); // calculate portion without master
            low_bound = (i - 1) * portion;

            if (((i + 1) == npes) && ((IM_SIZE % (npes - 1)) != 0)) {//if rows of [A] cannot be equally divided among slaves
	...
	MPI_Bcast(&coeff, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD); /* 8/4/2013 */
